...
public class Day19 implements IDay {
	...
	long part(LinkedList<String> data, int part) {
		...
		for (var el : scanners) {
			for (var er : scanners) {
				if (el.getScannerPosition().minus(er.getScannerPosition()).distanceFromZero() > distanceMax) {
					distanceMax = el.getScannerPosition().minus(er.getScannerPosition()).distanceFromZero();
				}
			}
		}
		if (part == 1)
